        ...
        {saldo > 0 && (
          <div className="bg-red-50 border border-red-100 rounded-2xl p-4 mb-6">
            ...
            <p className="text-3xl font-black tracking-tighter text-red-600">
              {fmt(valorParcela)}
              <span className="text-sm font-bold text-red-400"> /mês</span>
            </p>
            <p className="text-[9px] text-red-400 mt-1">
              Simulação sem juros. Taxa final sujeita à análise de crédito.
            </p>
          </div>
        )}
...
function VitrinePublicaInner() {
  ...
  const modelos = useMemo(
    () =>
      [
        ...new Set(
          estoque
            .filter((c) => !filtroMarca || c.marca === filtroMarca)
            .map((c) => c.modelo)
            .filter(Boolean)
        ),
      ].sort(),
  ...
  const anos = useMemo(
    () =>
      [...new Set(estoque.map((c) => c.ano_modelo).filter(Boolean))].sort(
        (a, b) => b - a
      ),
    [estoque]
  );
  ...
  const estoqueFiltrado = useMemo(
    () =>
      estoque.filter((c) => {
        ...
        if (filtroPreco && (c.preco_sugerido ?? 0) > parseInt(filtroPreco))
          return false;
        return true;
      }),
  ...
  const filtrosAtivos =
    filtroMarca || filtroModelo || filtroAno || filtroPreco;
  ...
  const selectClass =
    "appearance-none bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-bold text-gray-700 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 pr-8 w-full cursor-pointer";
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              {filtrosAtivos && (
                <button
                  ...
                  className="ml-auto text-[10px] font-black uppercase tracking-widest text-red-500 hover:text-red-600 flex items-center gap-1"
                >
                  <X size={11} /> Limpar
                </button>
              )}
        ...
        {estoqueFiltrado.length > 0 ? (
          <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {estoqueFiltrado.map((carro) => {
              ...
              // Selos condicionais — ativar quando dados vierem do backend
                ...
                >
                  ...
                  <Link href={`/vitrine/${carro.id}`} className="block relative aspect-video overflow-hidden bg-gray-100 flex-shrink-0">
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                      <span className="bg-white/90 backdrop-blur-sm text-gray-700 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">
                        Verificado
                      </span>
                      {/* Ativar quando dado existir */}
                      {unicoDono && (
                        <span className="bg-blue-600 text-white px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">
                          Único Dono
                        </span>
                      )}
                    </div>
          ...
          <div className="py-32 text-center border-2 border-dashed border-gray-200 rounded-3xl bg-white">
            ...
            {filtrosAtivos && (
              <button
                onClick={limparFiltros}
                className="mt-4 text-[10px] font-black uppercase tracking-widest text-red-500 hover:text-red-600"
              >
                Limpar filtros
              </button>
            )}
