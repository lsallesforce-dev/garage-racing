...
// Layout PREMIUM da vitrine — página do veículo.
//
// Existe pra a página do carro ter o MESMO cromo da home premium (topbar, menu,
// rodapé): card premium que leva pra uma página com outra cara derruba a
// impressão inteira. O layout padrão segue em `VitrineDetalheClient.tsx`,
// intocado.
//
// Como na home: nenhuma estimativa de parcela (ver commit c379051) — o caminho
// de financiamento é a FichaFinanciamento.

import { useMemo, useState } from "react";
import Link from "next/link";
import {
  ChevronLeft, ChevronRight, Gauge, Fuel, Cog, Palette, Play, Video, Car,
  MessageCircle, Check, Calculator, Settings2, Sparkles, ShieldCheck, MapPin,
} from "lucide-react";
import {
...
import { PremiumTopo, PremiumRodape, localDaLoja, type LojaPremium } from "@/components/vitrine/premium/Chrome";
...
export default function VitrineDetalhePremiumClient({
  ...
  // `null` = ficha aberta pelo menu (sem carro); objeto = ficha deste carro.
  ...
  const ficha_: [string, string | null][] = [
  ...
  const fichaPreenchida = ficha_.filter(([, v]) => v);
  ...
  const imgAtual = galeria[sel as number] ?? capa ?? "";
  ...
  return (
    <div style={themeStyle(theme)} className="min-h-screen bg-[var(--bg)] text-[var(--fg)] font-sans pb-24 lg:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-5 py-6">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-[var(--fg-faint)] mb-5">
          <Link href={`/vitrine/${tenant}`} className="flex items-center gap-1 hover:text-[var(--brand)] transition-colors">
            <ChevronLeft size={13} /> Estoque
          </Link>
          <ChevronRight size={12} />
          <span className="truncate text-[var(--fg-muted)]">{titulo}</span>
        </nav>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* ── Mídia ── */}
          {/* min-w-0: sem isso o item do grid (min-width:auto) cresce pra caber a
              tira de miniaturas inteira → overflow horizontal no mobile. */}
          <div className="lg:col-span-7 min-w-0">
            <div className="relative aspect-[4/3] rounded-3xl overflow-hidden bg-[var(--surface-2)] border border-[var(--border)]">
              {sel === "video" && videoUrl ? (
                ...
                <>
                  <img src={imgAtual} alt="" aria-hidden className="absolute inset-0 w-full h-full object-cover scale-110 blur-2xl opacity-70" />
                  <img src={imgAtual} alt={titulo} className="absolute inset-0 w-full h-full object-contain" />
                </>
              ) : (
            ...
            {(galeria.length > 1 || videoUrl) && (
              <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
                {videoUrl && (
                  <button onClick={() => setSel("video")} aria-label="Ver vídeo"
          ...
          <div className="lg:col-span-5 min-w-0">
            <div className="lg:sticky lg:top-40">
              <div className="flex flex-wrap items-center gap-1.5 mb-3">
                ...
                {selos.map((s) => <Chip key={s.key} className={s.className}>{s.labelLongo ?? s.label}</Chip>)}
              </div>
              ...
              {!vendido && (
                <div className="mt-4 flex flex-col gap-2.5">
                  ...
                  <button onClick={() => setFicha({ carro: veiculo })}
        ...
        {relacionados.length > 0 && (
          <section className="mt-16">
            <h2 className="text-[15px] font-black uppercase tracking-widest mb-5">
