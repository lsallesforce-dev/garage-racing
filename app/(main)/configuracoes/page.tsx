        ...
        .upsert(
          {
            ...(config.id ? { id: config.id } : {}),
            user_id: user.id,
            // nome_empresa e nome_agente são NOT NULL — trima mas não vira null.
            nome_empresa: config.nome_empresa.trim(),
            nome_fantasia: limpo(config.nome_fantasia),
            cnpj: limpo(config.cnpj),
            cidade: limpo(config.cidade),
            estado: limpo(config.estado),
            nome_agente: config.nome_agente.trim(),
            endereco: config.endereco.trim(),
            endereco_complemento: limpo(config.endereco_complemento),
            ...
            meta_phone_id: limpo(config.meta_phone_id),
            meta_access_token: limpo(config.meta_access_token),
            nome_usuario: limpo(config.nome_usuario),
            cargo_usuario: limpo(config.cargo_usuario),
            endereco_convite_ativo: !!config.endereco_convite_ativo,
            tom_venda: limpo(config.tom_venda),
            instrucoes_adicionais: limpo(config.instrucoes_adicionais),
            horario_funcionamento: limpo(config.horario_funcionamento),
            // textarea com só um "\n" é truthy e injetava um bloco de OFERTA
            // ESPECIAL vazio no prompt do agente — limpo() derruba pra null.
            oferta_especial:      limpo(config.oferta_especial),
