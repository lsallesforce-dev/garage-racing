...
export default function CarrosVitrine({ carros, totalLojas }: { carros: PortalCarro[]; totalLojas: number }) {
  ...
  // Troca automática a cada 5s (pausa no hover).
  useEffect(() => {
    if (destaques.length <= 1 || pausado) return;
    const t = setInterval(() => setDestaqueIdx((i) => (i + 1) % destaques.length), 5000);
            ...
            <div className="relative" onMouseEnter={() => setPausado(true)} onMouseLeave={() => setPausado(false)}>
              ...
              {destaques.length > 1 && (
                <>
                  ...
                  {/* Dots — indicam e trocam o carro (embaixo do card) */}
                  <div className="flex justify-center gap-2 mt-6">
                    {destaques.map((_, i) => (
                      <button key={i} onClick={() => setDestaqueIdx(i)} aria-label={`Ir pro carro ${i + 1}`}
                        className={`h-2.5 rounded-full transition-all ${i === destaqueIdx ? "w-7 bg-red-600" : "w-2.5 bg-gray-300 hover:bg-gray-400"}`} />
                    ))}
                  </div>
                </>
              )}
